...
def test_and_generate_animation(objective_function, generate_animation=True):
    pso_init_params = {'func':objective_function, 'dim': 2,
                    'pop': 50, 'max_iter': 50, 'w': 0.8, 'c1': 2, 'c2':2,
                    'lb':[-2.048, -2.048], 'ub':[2.048, 2.048],
                    'enable_learning_factors': True}  
    # 如果要启用学习因子，需要将上面的 enable_learning_factors 设置为 True，否则以下配置无效
    learning_factors_params = {'mode': 'asynchronous', 'c1_start': 3.05, 'c1_end': 1, 'c2_start': 1.05, 'c2_end': 3}
    #constriction：21
    #
    # 初始化粒子群算法
    ...
    # 三种不同的结束迭代的方式
    #pso.run(convergence_threshold=-428.72070399, rel_tol=1e-5) # 设置收敛阈值作为结束迭代的条件
    ...
    # 绘制优化过程中全局最优解的变化
    plt.rcParams['font.sans-serif'] = ['Heiti TC']  # 指定使用的中文字体
    ...
    if generate_animation  and len(pso.gbest_y_hist) < 100:
        # 绘制动画
        x1_lim=(-2.048-0.6, 2.048+0.6) # 将GIF动画的x轴扩展0.6
        x2_lim=(-2.048-0.6, 2.048+0.6) # 将GIF动画的y轴扩展0.6
        animation = plot_animation(pso, objective_function, '粒子群算法优化', xlim=x1_lim, ylim=x2_lim,arrow_length=0.1, dpi=100, frames_per_iter=10)
        # 保存动画为GIF文件
        animation.save('粒子群算法优化.gif', writer='pillow')
...
def run_pso_with_stats(pso_init_params, learning_factors_params, weight_modes, total_runs_per_mode=100):
    ...
    for weight_mode_params in weight_modes:
        ...
        for _ in range(total_runs_per_mode):
            ...
            # 运行 PSO 算法
            # pso.run(convergence_threshold=-428.72070399) # 设置收敛阈值作为结束迭代的条件
            # pso.run(precision=1e-7, N=10) # 当种群的适应度范围，连续10次最大适应度值 - 最小适应度值 < precision，结束迭代
...
# 使用 scipy.optimize 模块中的 minimize 函数来求解这个目标函数的极大值
# 注意：这个函数也是求解的局部最优
def optimize(objective_function):
    ''' 使用 minimize 函数求解目标函数的最优解 '''
    bounds = [(-2.048, 2.048), (-2.048, 2.048)] # 设置变量的取值范围    
    initial_guess = [-2.048,0.009] # 设置变量的初始猜测值    
    ...
    # 输出结果
    print("最优解：", max_x) # 最优解： [-2.048 0    ]
    print("最优目标值：", max_value) # 最优目标值： 428.72070399
...
if __name__ == "__main__":
    ...
    #   - 如果你需要设置不同的终止迭代的方式，可在调用 pso.run 方法时，传入不同的参数实现（见第28~31行，注释信息为“# 三种不同的结束迭代的方式”）
    ...
    pso_init_params = {'func':objective_function, 'dim': 2,
                        'pop': 20, 'max_iter': 100, 'w': 0.8, 'c1': 2, 'c2':2,
                        'lb':[-2.048, -2.048], 'ub':[2.048, 2.048],
                        'enable_learning_factors': True}         

    # 如果要启用学习因子，需要将上面的 enable_learning_factors 设置为 True，否则以下配置无效
    learning_factors_params_mode = {'c1_start':3, 'c1_end':1, 'c2_start':1, 'c2_end':3}
    ...
    # plot_3d_surface(objective_function, '目标函数1的三维图像', xlim=x1_lim, ylim=x2_lim, dpi=300)